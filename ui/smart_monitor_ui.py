...
from engines.smart_monitor_engine import SmartMonitorEngine
    ...
    for task in tasks:
        with st.container():
            ...
            if has_position and position_cost > 0 and position_quantity > 0:
                try:
                    # 获取实时行情
                    from data_sources.smart_monitor_data import SmartMonitorDataFetcher
...
def _render_task_kline_and_decisions(task: Dict, db: SmartMonitorDB, engine):
    ...
    Args:
        ...
        engine: 监控引擎实例
    """
    from interfaces.smart_monitor_kline import SmartMonitorKline
    from data_sources.smart_monitor_data import SmartMonitorDataFetcher
