...
class NewsFlowEngine:
    ...
    def _init_modules(self):
        """初始化所有模块"""
        try:
            from data_sources.news_flow_data import NewsFlowDataFetcher
            self.fetcher = NewsFlowDataFetcher()
        except Exception as e:
            logger.error(f"数据获取模块初始化失败: {e}")
        
        try:
            from strategies.news_flow_model import NewsFlowModel
            self.model = NewsFlowModel()
        except Exception as e:
            logger.error(f"流量模型模块初始化失败: {e}")
        
        try:
            from strategies.news_flow_sentiment import SentimentAnalyzer
        ...
        try:
            from strategies.news_flow_alert import NewsFlowAlertSystem
                ...
                if not self.agents:
                    ...
                    if ai_analysis and self.db and quick_result.get('snapshot_id'):
                        ai_record = {
                            ...
                            'model_used': getattr(self.agents, 'model', 'unknown'),
    ...
    def get_dashboard_data(self) -> Dict:
        ...
        try:
            ...
            # 调度器状态
            try:
                from schedulers.news_flow_scheduler import news_flow_scheduler
